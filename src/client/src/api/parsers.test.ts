...
describe("API parsers", () => {
  ...
  it("parses PI WEB runtime responses including the daemon-owned active profile", () => {
    expect(parsePiWebRuntimeResponse({
      ...
      generatedAt: "now",
      components: {
        web: { component: "web", label: "Web/UI", runtimeVersion: "1.0.0", available: true, capabilities: [PI_WEB_CAPABILITIES.sessionsAskUser, PI_WEB_CAPABILITIES.piPackagesManage, "future.capability"] },
        sessiond: {
          component: "sessiond",
      ...
      capabilities: [PI_WEB_CAPABILITIES.sessionsAskUser, PI_WEB_CAPABILITIES.piPackagesManage, "future.capability"],
    })).toMatchObject({
      capabilities: [PI_WEB_CAPABILITIES.sessionsAskUser, PI_WEB_CAPABILITIES.piPackagesManage],
