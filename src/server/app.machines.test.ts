  ...
  it("reports effective machine runtime capabilities for remote machines", async () => {
    ...
    const requestJson = vi.fn<MachineClient["requestJson"]>(() => Promise.resolve({
      ...
      body: {
        ...
        generatedAt: "2026-05-25T00:00:00.000Z",
        components: {
          web: { component: "web", label: "Remote Web", runtimeVersion: "1.0.0", available: true, capabilities: [PI_WEB_CAPABILITIES.workspaceFileSuggestions, PI_WEB_CAPABILITIES.piPackagesManage, PI_WEB_CAPABILITIES.agentProfileConfig, "future.capability"] },
          sessiond: {
            component: "sessiond",
        ...
        capabilities: [PI_WEB_CAPABILITIES.workspaceFileSuggestions, PI_WEB_CAPABILITIES.piPackagesManage, PI_WEB_CAPABILITIES.agentProfileConfig, "future.capability"],
      },
    }));
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
    ...
    expect(runtime.json()).toMatchObject({
      machineId: remote.id,
      ok: true,
      capabilities: [PI_WEB_CAPABILITIES.workspaceFileSuggestions, PI_WEB_CAPABILITIES.piPackagesManage, PI_WEB_CAPABILITIES.agentProfileConfig],
