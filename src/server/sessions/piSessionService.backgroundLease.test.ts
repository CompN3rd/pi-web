...
describe("PiSessionService plugin-owned background leases", () => {
  it("blocks interactive mutation while the attributed lease is active and releases after completion", async () => {
    ...
    const created = await service.startBackgroundSession("automations", "/workspace", {});
    const ref = sessionRef(created.session.id);

    await expect(service.prompt(ref, "interactive")).rejects.toThrow("automations lease is active");
    await expect(service.stop(ref)).rejects.toThrow("automations lease is active");
    await expect(service.promptBackgroundSession("other", ref, "wrong owner")).rejects.toThrow("not active for plugin other");

    await expect(service.promptBackgroundSession("automations", ref, "scheduled work")).resolves.toMatchObject({ sessionId: "session-1" });
    service.releaseBackgroundSession("automations", ref);
  ...
  it("force-detaches without awaiting stalled runtime disposal", async () => {
    ...
    await expect(service.forceStopBackgroundSession("automations", sessionRef(created.session.id))).resolves.toBeUndefined();
