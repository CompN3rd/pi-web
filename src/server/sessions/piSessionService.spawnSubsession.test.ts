  ...
  describe("spawnSubsession", () => {
    ...
    it("hydrates persisted child links after a service restart so the parent can inspect them", async () => {
      ...
      modelRuntime: testModelRuntime,
          ...
          sessionManager: { create: () => parent.session.sessionManager, list: () => Promise.resolve([]), listAll: () => Promise.resolve([]), invalidateSessionFile: () => undefined, open },
          archiveStore: emptyArchiveStore(),
          heartbeatIntervalMs: 60_000,
        });
    ...
    it("ignores stale persisted child links when the child no longer records the parent", async () => {
      ...
      modelRuntime: testModelRuntime,
          ...
          sessionManager: { create: () => parent.session.sessionManager, list: () => Promise.resolve([]), listAll: () => Promise.resolve([]), invalidateSessionFile: () => undefined, open: () => fakeSessionManager() },
          archiveStore: emptyArchiveStore(),
          heartbeatIntervalMs: 60_000,
        });
    ...
    it("does not invent subsession links from existing child session headers", async () => {
      ...
      modelRuntime: testModelRuntime,
        createAgentRuntime: runtimeCreator(parent.runtime),
        sessionManager: { create: () => parent.session.sessionManager, list: () => Promise.resolve([]), listAll: () => Promise.resolve([childRecord]), invalidateSessionFile: () => undefined, open: () => fakeSessionManager() },
        archiveStore: emptyArchiveStore(),
        heartbeatIntervalMs: 60_000,
      });
    ...
    it("does not hydrate copied parent links when the opened parent has a different id", async () => {
      ...
      modelRuntime: testModelRuntime,
        ...
        sessionManager: { create: () => forkedParent.session.sessionManager, list: () => Promise.resolve([]), listAll: () => Promise.resolve([]), invalidateSessionFile: () => undefined, open: () => fakeSessionManager() },
        archiveStore: emptyArchiveStore(),
        heartbeatIntervalMs: 60_000,
      });
