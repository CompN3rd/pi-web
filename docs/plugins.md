...
A missing source keeps the panel hidden; a malformed present source surfaces an error panel so it can be repaired. If both fixed source files exist, the plugin fails closed rather than selecting an implicit precedence. It declares `machineSpecific: true`, so a selected remote machine must expose its own matching plugin/extension package rather than reusing the gateway copy. The synthetic token/answer-draft path is historical fixture coverage; it is not used for the real pilot.
...
After adding, removing, or changing either source, reload the browser page and type `/reload` in each affected idle PI WEB session so plugin and tool registration matches the current valid source. Do the same after installing/updating the Pi extension, then use `/research-library` to confirm whether the historical synthetic tools or local-pilot read tools are available. The real pilot path does not insert text into prompts, submit prompts, create runtime dispatch intents, or publish an answer queue; the user starts an ordinary Pi session and the extension exposes read-only tools.

The local pilot renders one PDF page at a time with the packaged PDF.js worker over PI WEB's bounded same-origin range transport and provides an Open PDF fallback. It never modifies source notes or PDFs and exposes no send-to-agent button, prompt insertion, runtime intent, search-budget tool, or answer queue. The raw Pi extension's pilot mode exposes only bounded read tools: `research_library_list_papers`, `research_library_list_annotations` (optional paper/kind/status filters), and `research_library_get_annotation`. They return manifest metadata and digest-checked annotation records as untrusted quoted data; they do not write answers or promote content into the wiki. Agent disclosure for real paper content remains separately gated.
