// This runtime parser intentionally does not import browser plugin code. The
// two trust boundaries share contract vectors in tests but validate inputs
// independently so the installed raw extension never depends on plugin assets.
...
export function passageById(paper: SyntheticPaper, passageId: string): SyntheticPassage | undefined {
  return paper.passages.find((passage) => passage.id === passageId);
}
