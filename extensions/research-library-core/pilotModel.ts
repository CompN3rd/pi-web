...
export function parseLocalPilotConfig(value: unknown): ParseLocalPilotConfigResult {
  ...
  return { ok: true, config: { version: RESEARCH_LIBRARY_PILOT_VERSION, pilot: true, libraryId: libraryId.value, papers } };
}
...
function parsePilotPaper(value: unknown, index: number): { ok: true; paper: LocalPilotPaper } | { ok: false; error: string } {
  ...
  const sourceNotePath = sourceNotePathValue(value["sourceNotePath"], `${label} sourceNotePath`);
  if (!sourceNotePath.ok) return sourceNotePath;
  ...
  return {
    ...
    paper: {
      ...
      sourceNotePath: sourceNotePath.value,
...
function parsePilotPdf(value: unknown, paperLabel: string): { ok: true; pdf: LocalPilotPdf } | { ok: false; error: string } {
  ...
  const path = pdfPathValue(value["path"], `${label} path`);
...
function pdfPathValue(value: unknown, label: string): { ok: true; value: string } | { ok: false; error: string } {
...
function sourceNotePathValue(value: unknown, label: string): { ok: true; value: string } | { ok: false; error: string } {
  const result = normalizedRelativePath(value, label);
  if (!result.ok) return result;
